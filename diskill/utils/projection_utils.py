"""
From https://github.com/boschresearch/trust-region-layers/blob/main/trust_region_projections/utils/projection_utils.py.
Adapted to Episodic case (mainly naming).
"""
...
from utils.torch_utils import torch_batched_trace_square, torch_batched_trace
...
def gaussian_kl(policy: AbstractGaussianPolicy, p: Tuple[ch.Tensor, ch.Tensor],
    ...
    # trace_term = torch_batched_trace(prec_other @ cov)
    trace_term = torch_batched_trace_square(ch.linalg.solve_triangular(std_other, std, upper=False))
    cov_part = .5 * (trace_term - k + det_term_other - det_term)
...
def constraint_values(proj_type, policy: AbstractGaussianPolicy, p: Tuple[ch.Tensor, ch.Tensor],
    ...
    Args:
        ...
        policy: current policy
        p: mean and mean of gaussian p
        q: mean and mean of gaussian q
    ...
    if proj_type == "w2":
        ...
        # For this case only the sum is relevant, no individual projections for mean and mean make sense
