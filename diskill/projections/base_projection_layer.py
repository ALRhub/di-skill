"""
From https://github.com/boschresearch/trust-region-layers/blob/main/trust_region_projections/projections/base_projection_layer.py.
Only minor description changes.
"""
...
from utils.projection_utils import get_entropy_schedule, gaussian_kl
...
class BaseProjectionLayer(object):
    ...
    def get_trust_region_loss(self, policy: AbstractGaussianPolicy, p: Tuple[ch.Tensor, ch.Tensor],
        ...
        delta_loss = (mean_diff * int(policy.contextual) + cov_diff * int(policy.contextual_std)).mean()
    ...
    def trust_region_regression(self, policy: AbstractGaussianPolicy, obs: ch.Tensor, q: Tuple[ch.Tensor, ch.Tensor],
        ...
        Args:
            policy: policy instance
            obs: collected observations from data_handling
            q: old distributions
